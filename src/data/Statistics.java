...
public class Statistics {
	...
	public class GamePlayer implements Callable<Point> {
		...
		public Point call() throws Exception {
			...
			while (!moves.isEmpty()) {
				...
				if (System.currentTimeMillis() - start >= 80 || move == null) {
					game.move(game.getMoves().get(0));
		//			game.timeTrialMove();
				} else {
					game.move(move);
	...
	public void begin() {
		...
		for (Future<Point> future : list) {
			try {
				...
				if(i>0) {
					System.out.println((i++) + " " + result + " " + totalScore / i);
				}else{
	...
	private String calculatePossibleGrade() {
		double grade = 0.0;
		int minScore = 5000;
		int averageScore = 38000;
		if (mean < minScore) {
			return String.valueOf(grade);
		} else if (mean < averageScore) {
			grade = 20 + ((mean - minScore) / averageScore) * 30;
			return String.valueOf(grade);
		}
		grade = 50 + ((mean - averageScore) / mean) * 30;
		return String.valueOf(grade);
	}
	...
	public static void main(String[] args) {
		/*Statistics s = new Statistics(100
				, () -> new ai.RandomAI());
		s.begin();
		System.out.println(s);*/

		Statistics s = new Statistics(100
				, () -> new ai.JamesConway2048());
		s.begin();
		System.out.println(s);
